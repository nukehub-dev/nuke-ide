...
class BoundingBox:
    ...
    def from_dict(cls, data: dict) -> "BoundingBox":
        return cls(min=tuple(data["min"]), max=tuple(data["max"]))
...
class OverlapChecker:
    ...
    def find_overlaps(
        ...
        parallel: bool = False,
        progress_callback: Callable[[OverlapProgress], None] | None = None,
    ) -> list[OverlapResult]:
        """
        ...
        try:
            ...
            try:
                ...
                raw_overlaps = self.geometry.find_overlaps(
                    n_samples=sample_points, tolerance=tolerance, bbox=sampling_bounds
                )
                ...
                print(
                    f"[OverlapChecker] Warning: Built-in find_overlaps failed: {e}. Falling back to manual search.",
                    file=sys.stderr,
                )
            ...
            print(
                f"[OverlapChecker] Found {len(overlaps_found)} overlaps in {elapsed:.1f}s",
                file=sys.stderr,
            )
    ...
    def _find_overlaps_manual(
        ...
        progress_callback: Callable[[OverlapProgress], None] | None,
    ) -> list[OverlapResult]:
        ...
        samples = rng.uniform(
            low=[xmin, ymin, zmin], high=[xmax, ymax, zmax], size=(sample_points, 3)
        )
        ...
        for i, point in enumerate(samples):
            ...
            if len(cells_found) > 1:
                overlap = OverlapResult(
                    coordinates=tuple(point.tolist()), cell_ids=cells_found, cell_names=cell_names
                )
                overlaps_found.append(overlap)
    ...
    def get_overlap_viz_data(self, overlaps: list[OverlapResult], marker_size: float = 1.0) -> dict:
        ...
        return {"markers": markers, "overlappingCellIds": list(overlapping_cell_ids)}
...
def check_overlaps(
    ...
    try:
        ...
        if not materials_path.exists() and geom_path.suffix != ".py":
            print(
                f"[OverlapChecker] Creating dummy materials.xml at {materials_path}",
                file=sys.stderr,
            )

            # Scan geometry.xml for all referenced material IDs
            material_ids = {"1"}  # Always include 1 as default
            try:
                ...
                print(
                    f"[OverlapChecker] Warning: Could not parse geometry.xml to find material IDs: {e}",
                    file=sys.stderr,
                )

            # Create materials.xml with all found IDs
            mats_xml = ['<?xml version="1.0"?>', "<materials>"]
            for m_id in sorted(material_ids, key=lambda x: int(x) if x.isdigit() else x):
        ...
        try:
            ...
            overlaps = checker.find_overlaps(
                sample_points=sample_points, tolerance=tolerance, bounds=bbox, parallel=parallel
            )
        ...
        return {"overlaps": [], "totalOverlaps": 0, "samplesChecked": 0, "error": str(e)}


def get_overlap_viz_data(
    geometry_path: str, overlaps: list[dict], marker_size: float = 1.0
) -> dict:
    ...
    try:
        ...
        traceback.print_exc(file=sys.stderr)
        return {"markers": [], "overlappingCellIds": [], "error": str(e)}
...
if __name__ == "__main__":
    ...
    def progress_callback(progress: OverlapProgress):
        print(
            f"Progress: {progress.percentage}% ({progress.checked}/{progress.total}) - Found {len(progress.current_overlaps)} overlaps",
            file=sys.stderr,
        )

    checker = OverlapChecker(args.geometry)
    overlaps = checker.find_overlaps(
        sample_points=args.samples, tolerance=args.tolerance, progress_callback=progress_callback
    )

    result = {"overlaps": [o.to_dict() for o in overlaps], "totalOverlaps": len(overlaps)}
