...
class TallyInfo:
    ...
    mesh_dimensions: tuple | None = None
    mesh_bounds: dict[str, tuple[float, float]] | None = None
...
class OpenMCVTKExporter:
    ...
    def export_all_mesh_tallies(self, output_dir: str | None = None) -> list[MeshTallyData]:
                    ...
                    for nuclide in tally.nuclides:
                        try:
                            data = self.export_mesh_tally(
                                tally_id, score=score, nuclide=nuclide, output_dir=output_dir
                            )
                            results.append(data)
                        except Exception as e:
                            print(
                                f"Warning: Failed to export tally {tally_id} {score}/{nuclide}: {e}",
                                file=sys.stderr,
                            )
        return results

    def get_cell_tally_data(
        self, tally_id: int, score: str | None = None, nuclide: str | None = None
    ) -> CellTallyData:
        """
    ...
    def find_geometry_file(self, run_dir: str | None = None) -> str | None:
        """
    ...
    def _get_mesh_filter(self, tally: openmc.Tally) -> openmc.MeshFilter | None:
    ...
    def _resolve_score(self, tally: openmc.Tally, score: str | None) -> tuple[int, str]:
        ...
        try:
            ...
            for i, s in enumerate(scores):
                if s.lower() == score.lower():
                    return i, s
            print(
                f"Warning: Score '{score}' not found in tally, using first score '{scores[0]}'",
                file=sys.stderr,
            )
            return 0, scores[0]

    def _resolve_nuclide(self, tally: openmc.Tally, nuclide: str | None) -> tuple[int, str]:
        ...
        try:
            ...
            print(
                f"Warning: Nuclide '{nuclide}' not found in tally, using first nuclide '{nuclides[0]}'",
                file=sys.stderr,
            )
            return 0, nuclides[0]
    ...
    def create_structured_grid(
        self, tally_id: int, pixelated: bool = False
    ) -> tuple[Any, str, tuple[float, float]]:
        """
...
def export_statepoint_tallies(
    statepoint_path: str, output_dir: str | None = None, tally_ids: list[int] | None = None
            ...
            for score in tally_info.scores:
                for nuclide in tally_info.nuclides:
                    data = exporter.export_mesh_tally(
                        tally_info.id, score=score, nuclide=nuclide, output_dir=output_dir
                    )
                    results.append(data)
        except Exception as e:
            errors.append(f"Tally {tally_info.id}: {e}")

    return {"mesh_tallies": results, "errors": errors}
