...
class OpenMCReader:
    ...
    def load_geometry(self, h5m_path: str, filter_graveyard: bool = True):
        ...
        result = convert_h5m_to_vtk_cached(
            h5m_path, use_cache=True, do_filter_graveyard=filter_graveyard
        )
    ...
    def load_statepoint(self, statepoint_file: str) -> dict:
        ...
        with h5py.File(statepoint_file, "r") as f:
            info = {
                ...
                "generations_per_batch": int(f["n_realizations"][()])
            ...
            if "tallies" in f:
                ...
                tally_ids = [
                    int(key.split()[-1]) for key in tallies_group.keys() if key.startswith("tally ")
                ]
            ...
            for key in tally_group.keys():
                ...
                if "score_bins" in tally:
                    ...
                    if isinstance(val, np.ndarray):
                        ...
                        val = [val]
                    tally_info["scores"] = [
                        v.decode("utf-8") if hasattr(v, "decode") else str(v) for v in val
                    ]
    ...
    def _parse_filter(self, filter_obj, tally_info: dict, f=None) -> None:
        ...
        if (
            ...
            and filter_type.startswith("b'")
            and filter_type.endswith("'")
        ):
        ...
        filter_info = {"type": filter_type, "bins": n_bins}
            ...
            if (
                ...
                if mesh_key in f["tallies"]["meshes"]:
                    ...
                    if "width" in mesh_obj:
                        ...
                        k in filter_info for k in ["mesh_dimensions", "lower_left", "upper_right"]
                    ):
                ...
                if mesh_key in f["tallies"]["meshes"]:
                    ...
                    if "dimension" in mesh_obj:
                        ...
                        if "width" in mesh_obj:
                            ...
                            k in mesh_info for k in ["dimensions", "lower_left", "upper_right"]
                        ):
    ...
    def create_mesh_tally_vtu(
        self, tally: OpenMCTally, score_index: int = 0, nuclide_index: int = 0
    ...
    def _create_cylindrical_mesh_vtu(
        self, tally: OpenMCTally, score_index: int = 0, nuclide_index: int = 0
    ) -> vtk.vtkUnstructuredGrid:
    ...
    def load_source(self, source_file: str, max_particles: int = None) -> vtk.vtkPolyData:
        ...
        with h5py.File(source_file, "r") as f:
            if "source_bank" in f:
                ...
                raise ValueError(
                    f"No 'source' group or 'source_bank' dataset found in {source_file}. "
    ...
    def visualize_tally_on_geometry(
        self,
        geometry_file: str,
        statepoint_file: str,
        tally_id: int,
        ...
        if geometry_file.endswith(".h5m"):
            ...
            result = convert_h5m_to_vtk_cached(
                geometry_file, use_cache=False, do_filter_graveyard=filter_graveyard
            )
            ...
            try:
                ...
                if dagmc_elem is not None:
                    ...
                    if dagmc_filename:
                        ...
                        if not os.path.isabs(dagmc_filename):
                            dagmc_filename = os.path.join(
                                os.path.dirname(geometry_file), dagmc_filename
                            )
                        ...
                        result = convert_h5m_to_vtk_cached(
                            dagmc_filename, use_cache=False, do_filter_graveyard=filter_graveyard
                        )
                        geometry = simple.OpenDataFile(result["vtk_path"])
                    else:
                    ...
                    from plugins.openmc.lib.geometry_viz import OpenMCGeometryVisualizer
        ...
        if has_cell_filter:
            ...
            result = self._apply_cell_tally_to_geometry(geometry, tally, statepoint_file, score)
    ...
    def visualize_tally_and_source_on_geometry(
        ...
        result = self.visualize_tally_on_geometry(
            geometry_file, statepoint_file, tally_id, score, filter_graveyard=filter_graveyard
        )
    ...
    def _validate_spatial_overlap(self, geometry, tally) -> str | None:
        """
        ...
        try:
            ...
            overlap_x = max(
                0,
                min(geom_bounds["x"][1], mesh_bounds["x"][1])
                - max(geom_bounds["x"][0], mesh_bounds["x"][0]),
            )
            overlap_y = max(
                0,
                min(geom_bounds["y"][1], mesh_bounds["y"][1])
                - max(geom_bounds["y"][0], mesh_bounds["y"][0]),
            )
            overlap_z = max(
                0,
                min(geom_bounds["z"][1], mesh_bounds["z"][1])
                - max(geom_bounds["z"][0], mesh_bounds["z"][0]),
            )

            # Calculate overlap volume
            geom_volume = (
                (geom_bounds["x"][1] - geom_bounds["x"][0])
    ...
    def _map_tally_to_cells_spatial(
        self, geometry_vtk, tally, mesh_info: dict, score_idx: int = 0
    ) -> vtk.vtkUnstructuredGrid:
        """
    ...
    def _apply_mesh_tally_to_geometry(self, geometry, tally, score: str = None) -> dict:
        ...
        try:
            ...
            print(
                f"[Overlay] Mesh source cell arrays: {[cell_data.GetArray(i).GetName() for i in range(cell_data.GetNumberOfArrays())]}",
                file=sys.stderr,
            )
            print(
                f"[Overlay] Mesh source point arrays: {[point_data.GetArray(i).GetName() for i in range(point_data.GetNumberOfArrays())]}",
                file=sys.stderr,
            )
            ...
            if cell_data.GetArray("tally_mean"):
                ...
                print(
                    f"[Overlay] Mesh tally_mean range: [{rng[0]:.6e}, {rng[1]:.6e}]",
                    file=sys.stderr,
                )
            # Check mesh bounds
        ...
        resampled = simple.ResampleWithDataset(
            SourceDataArrays=mesh_source, DestinationMesh=geometry
        )
        ...
        try:
            ...
            print(
                f"[Overlay] Resampled cell arrays: {[cell_data.GetArray(i).GetName() for i in range(cell_data.GetNumberOfArrays())]}",
                file=sys.stderr,
            )
            print(
                f"[Overlay] Resampled point arrays: {[point_data.GetArray(i).GetName() for i in range(point_data.GetNumberOfArrays())]}",
                file=sys.stderr,
            )
            ...
            for i in range(point_data.GetNumberOfArrays()):
                ...
                if arr and "tally" in arr.GetName():
                    rng = arr.GetRange()
                    print(
                        f"[Overlay] Resampled '{arr.GetName()}' range: [{rng[0]:.6e}, {rng[1]:.6e}]",
                        file=sys.stderr,
                    )
        ...
        if not resampled_valid:
            print(
                "[Overlay] Resampling produced no valid data, trying spatial cell mapping...",
                file=sys.stderr,
            )
            try:
                ...
                resampled_valid = True
                print("[Overlay] Spatial cell mapping succeeded", file=sys.stderr)

            except Exception as e:
                print(
                    f"[Overlay] Spatial cell mapping failed: {e}, falling back to mesh overlay",
                    file=sys.stderr,
                )
                import traceback
                ...
                for i in range(data.GetNumberOfArrays()):
                    ...
                    if arr:
                        ...
                        if name and "tally_mean" in name:
                            ...
                            array_exists = True
                            print(
                                f"[Overlay] Found tally array: '{name}' in {attr_name}",
                                file=sys.stderr,
                            )
                            # Check its range
                            rng = arr.GetRange()
                            print(
                                f"[Overlay] Array range: [{rng[0]:.6e}, {rng[1]:.6e}]",
                                file=sys.stderr,
                            )
                            break
                if array_exists:
        ...
        if not array_exists:
            print(
                "[Overlay] Warning: No tally_mean array found in resampled data!", file=sys.stderr
            )
            print("[Overlay] Available arrays will be listed above", file=sys.stderr)
            ...
            try:
                ...
                if data_array:
                    ...
                    if data_range[0] == data_range[1]:
                        print(
                            f"[Overlay] Warning: Zero data range [{data_range[0]:.6e}], using [0, 1]",
                            file=sys.stderr,
                        )
                    ...
                    print(
                        f"[Overlay] Color range: [{data_range[0]:.6e}, {data_range[1]:.6e}]",
                        file=sys.stderr,
                    )
            except Exception as e:
                print(
                    f"[Overlay] Warning: Could not rescale transfer function: {e}", file=sys.stderr
                )
                # Fallback to automatic rescaling
                geometry_display.RescaleTransferFunctionToDataRange(True, False)
        else:
            print("[Overlay] ERROR: Could not find tally array for coloring", file=sys.stderr)
    ...
    def _apply_cell_tally_to_geometry(
        self, geometry, tally, statepoint_file: str, score: str = None
    ) -> dict:
        ...
        with h5py.File(statepoint_file, "r") as f:
                        ...
                        for fid in filter_ids.flat:
                            ...
                            if filter_key in filters_location:
                                ...
                                if "type" in filter_obj:
                                    val = filter_obj["type"][()]
                                    filter_type = (
                                        val.decode("utf-8") if hasattr(val, "decode") else str(val)
                                    )
            ...
            if filter_bins is None:
                print(
                    "[Overlay] No cell filter bins found, falling back to mesh overlay",
                    file=sys.stderr,
                )
                return self._show_mesh_overlay(geometry, tally)
            ...
            print(
                f"[Overlay] Found {len(cell_ids)} cells in cell filter: {cell_ids[:10]}...",
                file=sys.stderr,
            )
        ...
        for i, cell_id in enumerate(cell_ids):
            if i < len(tally.mean):
                ...
                if tally.mean.ndim >= 3:
                    value = tally.mean.flat[
                        i * len(tally.scores) * len(tally.nuclides)
                        + score_idx * len(tally.nuclides)
                    ]
    ...
    def _show_mesh_overlay(self, geometry, tally) -> dict:
        ...
        try:
            ...
            if data_array:
                ...
                lut.RescaleTransferFunction(data_range[0], data_range[1])
                print(
                    f"[Overlay] Mesh overlay color range: [{data_range[0]:.6e}, {data_range[1]:.6e}]",
                    file=sys.stderr,
                )
        except Exception as e:
...
class OpenMCPlotter:
    ...
    @staticmethod
    def create_energy_spectrum(
        statepoint_file: str, tally_id: int, score_index: int = 0, nuclide_index: int = 0
    ) -> dict[str, np.ndarray]:
        """
        ...
        with h5py.File(statepoint_file, "r") as f:
            ...
            for _i, filter_obj in enumerate(all_filters):
                filter_type = "unknown"
                if "type" in filter_obj:
                    val = filter_obj["type"][()]
                    filter_type = val.decode("utf-8") if hasattr(val, "decode") else str(val)
            ...
            return {"energy_bins": bins, "values": values, "std_dev": errors}
    ...
    def create_spatial_plot(
        ...
        if mesh.get("mesh_type") == "regular":
            ...
            x_centers = np.linspace(
                mesh["lower_left"][0] + dx / 2, mesh["upper_right"][0] - dx / 2, nx
            )
            y_centers = np.linspace(
                mesh["lower_left"][1] + dy / 2, mesh["upper_right"][1] - dy / 2, ny
            )
            z_centers = np.linspace(
                mesh["lower_left"][2] + dz / 2, mesh["upper_right"][2] - dz / 2, nz
            )
        else:
...
class OpenMCDepletionReader:
        ...
        with h5py.File(file_path, "r") as f:
            ...
            for key in sorted(f["materials"].keys()):
                try:
                    ...
                    print(
                        f"[DepletionReader] Warning: Could not read material {key}: {e}",
                        file=sys.stderr,
                    )
                    continue
    ...
    def load_material_data(
        file_path: str, material_index: int, nuclide_filter: list[str] | None = None
    ) -> dict[str, Any]:
        """
    ...
    def _calculate_activity(
        nuclide_names: list[str], concentrations: np.ndarray, h5_file: h5py.File
    ) -> dict[str, Any]:
        """
