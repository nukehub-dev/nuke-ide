...
class OpenMCGeometryParser:
    ...
    def __init__(self):
        ...
        self.root_universe_id: int | None = None
    ...
    def parse(self, file_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            if file_name in non_geometry_files:
                return {
                    "error": f"{file_name} is not a geometry file. Please open geometry.xml instead."
                }
    ...
    def _parse_geometry_xml(self, file_path: Path) -> dict[str, Any]:
        ...
        if root_tag not in ("geometry", "model"):
            ...
            if not (has_cells or has_surfaces or has_lattices):
                return {
                    "error": f"Not a valid geometry file. Expected <geometry> root element or geometry elements (cell, surface, lattice), but found <{root.tag}>."
                }
                ...
                if dagmc_filename:
                    if not os.path.isabs(dagmc_filename):
                        dagmc_filename = os.path.join(
                            os.path.dirname(str(file_path)), dagmc_filename
                        )
                    return {
                        "error": f"This geometry file uses DAGMC geometry (dagmc_universe). Please use the DAGMC file directly: {dagmc_filename}"
                    }
            return {
                "error": "No cells found in geometry file. The file may be empty or not a valid OpenMC geometry file."
            }
    ...
    def _parse_surface(self, elem: ET.Element) -> None:
        ...
        self.surfaces[surf_id] = Surface(
            id=surf_id, type=std_type, coefficients=coeffs, boundary=boundary
        )
    ...
    def _extract_surfaces_from_region(self, region: str) -> list[int]:
        ...
        surface_ids = []
        tokens = (
            region.replace("(", " ").replace(")", " ").replace("|", " ").replace("&", " ").split()
        )
    ...
    def _organize_universes(self) -> None:
        ...
        for univ_id, cells in cells_by_universe.items():
            self.universes[univ_id] = Universe(id=univ_id, cells=cells, is_root=(univ_id == 0))
    ...
    def _build_response(self, file_path: str) -> dict[str, Any]:
        ...
        return {
            ...
            "lattices": [lattice.to_dict() for lattice in self.lattices.values()],
