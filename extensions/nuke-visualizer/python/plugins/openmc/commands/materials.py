...
@command("openmc.list-group-structures", help="List available group structures")
def cmd_list_group_structures(args):
    ...
    print(json.dumps({"structures": structures_array, "metadata": metadata}, indent=2))
    return 0


@command("openmc.list-thermal-materials", help="List available thermal scattering materials")
@arg("--cross-sections", help="Path to cross_sections.xml")
...
@command("openmc.list-nuclides", help="List available nuclides")
@arg("--cross-sections", help="Path to cross_sections.xml")
...
@command("openmc.materials", help="Parse materials.xml file")
@arg("file", help="Path to materials.xml")
def cmd_materials(args):
    """Parse and return materials from materials.xml file."""
    try:
        from plugins.openmc.lib.materials_parser import parse_materials_file
...
@command("openmc.material-cell-linkage", help="Get material-cell mapping")
@arg("materials_file", help="Path to materials.xml")
@arg("geometry_file", help="Path to geometry.xml")
def cmd_material_cell_linkage(args):
    """Get mapping of materials to cells that use them."""
    try:
        from plugins.openmc.lib.materials_parser import get_material_cell_linkage
...
@command("openmc.add-material", help="Add a material to materials.xml")
@arg("file", help="Path to materials.xml")
@arg("--material-xml", required=True, help="XML snippet of the material")
...
@command("openmc.mix-materials", help="Mix multiple materials")
@arg("file", help="Path to materials.xml")
@arg("--material-ids", required=True, help="Comma-separated material IDs to mix")
@arg("--fractions", required=True, help="Comma-separated mixing fractions")
@arg("--percent-type", choices=["ao", "wo", "vo"], default="ao", help="Fraction type")
@arg("--name", help="Name of the new material")
@arg("--id", type=int, help="ID of the new material")
    ...
    try:
        ...
        if percent_type == "vo":
            ...
            effective_density = sum(
                v * m.density for v, m in zip(fractions, mats_to_mix, strict=False)
            )
            ...
            for mat, vol_frac in zip(mats_to_mix, fractions, strict=False):
            ...
            if total_number_density > 0:
                nuclide_atom_fractions = {
                    nuc: n / total_number_density for nuc, n in nuclide_number_densities.items()
                }
            else:
                nuclide_atom_fractions = dict.fromkeys(nuclide_number_densities, 0)

            # Calculate effective number density (atom/b-cm)
            effective_number_density = (
                total_number_density / total_volume_frac if total_volume_frac > 0 else 0
            )
            ...
            # Calculate effective density
            effective_density = (
                sum(f * m.density for f, m in zip(fractions, mats_to_mix, strict=False))
                / total_mass_frac
            )
            ...
            for mat, mass_frac in zip(mats_to_mix, fractions, strict=False):
                for nuc_name, frac, frac_type in mat.nuclides:
                    if frac_type == "wo":
            ...
            for nuc_name, ao_weight in ao_weights.items():
                nuclide_atom_fractions[nuc_name] = (
                    ao_weight / total_ao_weight if total_ao_weight > 0 else 0
                )
            ...
            for mat, atom_frac in zip(mats_to_mix, fractions, strict=False):
                for nuc_name, frac, frac_type in mat.nuclides:
                    if frac_type == "ao":
        ...
        if percent_type == "wo":
            ...
            for nuc_name, _ao_frac in nuclide_atom_fractions.items():
                ...
                if nuc_name in nuclide_mass_fractions:
                    mixed_mat.add_nuclide(
                        nuc_name, nuclide_mass_fractions[nuc_name], percent_type="wo"
                    )
        else:
            ...
            mixed_mat.set_density(
                "atom/b-cm", effective_number_density if percent_type == "vo" else effective_density
            )
        ...
        if percent_type == "wo":
            ...
            for nuc_name, frac in nuclide_mass_fractions.items():
                nuclides.append({"name": nuc_name, "fraction": frac, "fractionType": "wo"})
        else:
            # Return atom fractions
            for nuc_name, frac in nuclide_atom_fractions.items():
                nuclides.append({"name": nuc_name, "fraction": frac, "fractionType": "ao"})

        # Thermal scattering - pass info but don't add (user must handle manually)
        thermal_scattering = []
        if sab_warnings:
            thermal_scattering = [
                {"name": w.split("contains ")[-1]} for w in sab_warnings if "contains " in w
            ]
