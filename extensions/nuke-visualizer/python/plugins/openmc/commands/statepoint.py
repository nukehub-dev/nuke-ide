...
try:
    import vtk  # noqa: F401  # availability probe: sets HAS_PARAVIEW
    from paraview import simple  # noqa: F401  # availability probe: sets HAS_PARAVIEW
    from vtk.util import numpy_support  # noqa: F401  # availability probe: sets HAS_PARAVIEW
...
@command("openmc.statepoint-info", help="Get full statepoint information")
    ...
    try:
        with h5py.File(args.statepoint, "r") as f:
            ...
            if "energy_mode" in f:
                emode = f["energy_mode"][()]
                result["energyMode"] = (
                    emode.decode("utf-8") if isinstance(emode, bytes) else str(emode)
                )
            ...
            if "runtime" in f:
                ...
                total_init = get_runtime_value(
                    runtime_group, "total initialization", "total_initialization", "init"
                )
                reading_xs = get_runtime_value(
                    runtime_group, "reading cross sections", "reading_cross_sections", "reading_xs"
                )
                inactive_batches = get_runtime_value(
                    runtime_group, "inactive batches", "inactive_batches"
                )
                active_batches = get_runtime_value(
                    runtime_group, "active batches", "active_batches"
                )
                simulation = get_runtime_value(runtime_group, "simulation", "active")
                transport = get_runtime_value(runtime_group, "transport")
                syn_fission_bank = get_runtime_value(
                    runtime_group, "synchronizing fission bank", "synchronizing_fission_bank"
                )
                source_sampling = get_runtime_value(
                    runtime_group, "sampling source sites", "sampling_source_sites"
                )
                tally_accum = get_runtime_value(
                    runtime_group, "accumulating tallies", "accumulating_tallies"
                )
                writing_sp = get_runtime_value(
                    runtime_group, "writing statepoints", "writing_statepoints"
                )
                send_recv = get_runtime_value(
                    runtime_group, "SEND-RECV source sites", "send_recv_source_sites"
                )

                # Get total - try various keys
                total_runtime = get_runtime_value(
                    runtime_group, "total", "total time", "total_time"
                )
                if total_runtime == 0:
            ...
            if "global_tallies" in f:
                ...
                global_scores = [
                    ...
                    "loss_to_fission",
                    "fission_neutrons",
                    "non_fission_captures",
                ]
                for i, (name, score) in enumerate(zip(global_names, global_scores, strict=True)):
            ...
            if "tallies" in f and "filters" in f["tallies"]:
                ...
                for key in filters_group.keys():
                    ...
                    result["filters"].append(
                        {"id": filter_id, "type": filter_type, "nBins": n_bins}
                    )
            ...
            if "tallies" in f and "meshes" in f["tallies"]:
                ...
                for key in meshes_group.keys():
                    ...
                    if "width" in mesh_obj:
                        ...
                        mesh_info["dimensions"]
                        and mesh_info["lowerLeft"]
                        and mesh_info["upperRight"]
                    ):
                        # Calculate width
...
@command("openmc.k-generation", help="Get k-generation data for convergence plot")
@arg("statepoint", help="Path to statepoint file")
def cmd_k_generation(args):
    ...
    try:
        with h5py.File(args.statepoint, "r") as f:
            ...
            upper_bound = [m + 2 * s for m, s in zip(cumulative_mean, cumulative_std, strict=True)]
            lower_bound = [m - 2 * s for m, s in zip(cumulative_mean, cumulative_std, strict=True)]
...
@command("openmc.source-data", help="Get source particle data")
@arg("statepoint", help="Path to statepoint file")
@arg("--max-particles", type=int, default=10000, help="Maximum particles to return")
...
@command("openmc.energy-distribution", help="Get energy distribution histogram")
@arg("statepoint", help="Path to statepoint file")
@arg("--bins", type=int, default=50, help="Number of energy bins")
