...
@command("openmc.xs-plot", help="Plot cross-sections")
@arg("--nuclides", help="Comma-separated nuclide names")
@arg("--reactions", required=True, help="Comma-separated reaction MT numbers")
@arg("--temperature", type=float, default=294.0, help="Temperature in Kelvin")
@arg("--energy-min", type=float, default=1e-5, help="Minimum energy in eV")
@arg("--energy-max", type=float, default=2e7, help="Maximum energy in eV")
@arg("--energy-region", help="Energy region preset")
@arg("--cross-sections", help="Path to cross_sections.xml file")
@arg("--temp-comparison", help="Temperature comparison mode")
@arg("--materials", help="JSON string of materials with components")
@arg("--flux-spectrum", help="JSON string of flux spectrum")
@arg("--library-comparison", help="JSON string of library comparison configuration")
@arg("--include-uncertainty", action="store_true", help="Include uncertainty/error data")
@arg("--include-integrals", action="store_true", help="Calculate and include integral quantities")
@arg(
    "--include-derivative", action="store_true", help="Calculate and include derivative/slope data"
)
@arg("--thermal-scattering", help="JSON string of thermal scattering request")
@arg("--chain-decay", help="JSON string of chain decay/buildup request")
@arg("--group-structure", help="Energy group structure name")
    ...
    try:
        ...
        try:
            if cross_sections_path:
                ...
                print(
                    f"[XS Plot] Loaded cross_sections.xml from: {cross_sections_path}",
                    file=sys.stderr,
                )
            else:
                library = openmc.data.DataLibrary.from_xml()
                print("[XS Plot] Loaded default cross_sections.xml", file=sys.stderr)
        except Exception as e:
        ...
        def load_nuclide_data(nuclide_name, library, xs_dir):
            ...
            print(
                f"[XS Plot] load_nuclide_data: Looking for {nuclide_name}, xs_dir={xs_dir}",
                file=sys.stderr,
            )
                ...
                for lib in libraries:
                    ...
                    for table in tables:
                        ...
                        if table_name in possible_names:
                            ...
                            print(
                                f"[XS Plot] Found nuclide {nuclide_name} in library table: {xs_file}",
                                file=sys.stderr,
                            )
                            break
                    if xs_file:
            ...
            if xs_file and os.path.exists(xs_file):
                ...
                try:
                    ...
                    print(
                        f"[XS Plot] Successfully loaded nuclide data from: {xs_file}",
                        file=sys.stderr,
                    )
                except Exception as e:
                ...
                print(
                    f"[XS Plot] XS file not found or not specified: xs_file={xs_file}, exists={os.path.exists(xs_file) if xs_file else 'N/A'}",
                    file=sys.stderr,
                )
        ...
        def extract_uncertainty_data(nuc_data, reaction_mt, temperature, energy_grid):
            ...
            print(
                f"[XS Plot] extract_uncertainty_data: MT={reaction_mt}, temp={temperature}",
                file=sys.stderr,
            )
            ...
            try:
                if reaction_mt not in nuc_data.reactions:
                    print(
                        f"[XS Plot] Reaction MT={reaction_mt} not found in nuc_data",
                        file=sys.stderr,
                    )
                    return None
                ...
                print(
                    f"[XS Plot] Found reaction MT={reaction_mt}, type={type(reaction)}",
                    file=sys.stderr,
                )
                ...
                print(
                    f"[XS Plot] Checking reaction.xs attribute: {hasattr(reaction, 'xs')}",
                    file=sys.stderr,
                )
                if hasattr(reaction, "xs") and reaction.xs:
                    print(
                        f"[XS Plot] reaction.xs type: {type(reaction.xs)}, keys: {list(reaction.xs.keys()) if hasattr(reaction.xs, 'keys') else 'N/A'}",
                        file=sys.stderr,
                    )
                    ...
                    print(
                        f"[XS Plot] Got xs_data for temp {closest_temp}, type: {type(xs_data)}",
                        file=sys.stderr,
                    )
                    ...
                    has_unc_attr = (
                        hasattr(xs_data, "_uncertainty") and xs_data._uncertainty is not None
                    )
                    if has_unc_attr:
                    ...
                    # Check for yield/multiplicity uncertainty (common for fission)
                        ...
                        rxn_yield = reaction.yield_
                        if (
                            rxn_yield is not None
                            and hasattr(rxn_yield, "std_dev")
                            and rxn_yield.std_dev is not None
                        ):
                            y = (
                                getattr(rxn_yield, "y", xs_data.y)
                                if hasattr(rxn_yield, "y")
                                else xs_data.y
                            )
                    ...
                    if uncertainty is None:
                        print(
                            f"[XS Plot] No uncertainty data available for MT={reaction_mt} in this library",
                            file=sys.stderr,
                        )
        ...
        def calculate_derivative(curve):
            ...
            try:
                ...
                with np.errstate(divide="ignore", invalid="ignore"):
                    ...
                    log_log_deriv[valid_mask] = (energy[valid_mask] / xs[valid_mask]) * dXdE[
                        valid_mask
                    ]
                ...
                print(
                    f"[XS Plot] Derivative calculated: max slope = {max_slope:.4e} at {max_slope_energy:.4e} eV",
                    file=sys.stderr,
                )
        ...
        def calculate_chain_decay(
            curve, parent_nuclide, decay_time, include_daughters, max_depth, xs_dir
        ):
            ...
            try:
                ...
                if include_daughters and daughters and decay_time > 0:
                    ...
                    for i, daughter in enumerate(daughters[:max_depth]):
                        try:
                                    ...
                                    if hasattr(reaction, "xs") and reaction.xs:
                                        ...
                                        if hasattr(xs_data, "x") and hasattr(xs_data, "y"):
                                            ...
                                            daughter_xs = np.interp(
                                                energy, xs_data.x, xs_data.y, left=0, right=0
                                            )
                                            ...
                                            contributions[daughter] = (
                                                daughter_xs * branch_frac * buildup
                                            )
                            ...
                            print(
                                f"[XS Plot] Could not load daughter {daughter}: {e}",
                                file=sys.stderr,
                            )
                            continue
                ...
                for _nuc, xs in contributions.items():
                    cumulative += xs
                ...
                if include_derivative:
                    print(
                        "[XS Plot] Calculating derivative for chain decay cumulative XS",
                        file=sys.stderr,
                    )
                    # Create a temporary curve-like dict for derivative calculation
                    temp_curve = {"energy": curve.get("energy", []), "xs": cumulative_list}
                    chain_data["derivative"] = calculate_derivative(temp_curve)

                print(
                    f"[XS Plot] Chain decay calculated: {parent_nuclide} -> {daughter_nuclides}",
                    file=sys.stderr,
                )
        ...
        def calculate_multigroup_xs(curve, group_structure):
            ...
            try:
                ...
                print(
                    f"[XS Plot] Multigroup XS calculated: {num_groups} groups for {curve.get('nuclide', 'unknown')}",
                    file=sys.stderr,
                )
        ...
        def load_thermal_scattering_data(material_name, xs_dir):
            ...
            if available_materials and material_name not in available_materials:
                print(
                    f"[XS Plot] Material '{material_name}' not found in cross_sections.xml",
                    file=sys.stderr,
                )
                print(
                    f"[XS Plot] Available thermal materials: {', '.join(available_materials)}",
                    file=sys.stderr,
                )
                return None
            ...
            if ts_file is None:
                print(
                    f"[XS Plot] Could not find thermal scattering file for {material_name}",
                    file=sys.stderr,
                )
                if available_materials:
                    print(
                        f"[XS Plot] Available materials: {', '.join(available_materials)}",
                        file=sys.stderr,
                    )
                return None
        ...
        def get_thermal_scattering_xs(ts_data, temperature, energy_min, energy_max):
            ...
            print(
                f"[XS Plot] Using thermal scattering data at {closest_temp_k:.1f}K (requested {temperature}K)",
                file=sys.stderr,
            )
                ...
                if hasattr(ts_data, "inelastic") and hasattr(ts_data.inelastic, "xs"):
                    ...
                    if temp_key in inelastic_dict:
                        ...
                        if inelastic_dict:
                            ...
                            if hasattr(xs_data, "x") and hasattr(xs_data, "y"):
                                inelastic_xs = np.interp(
                                    energy, xs_data.x, xs_data.y, left=0, right=0
                                )
                                print(
                                    f"[XS Plot] Loaded inelastic XS using {first_key} (requested {temp_key} not found)",
                                    file=sys.stderr,
                                )
                ...
                if hasattr(ts_data, "elastic") and hasattr(ts_data.elastic, "xs"):
                    ...
                    if temp_key in elastic_dict:
                        ...
                        if hasattr(xs_data, "bragg_edges") and hasattr(xs_data, "factors"):
                            elastic_xs = np.interp(
                                energy, xs_data.bragg_edges, xs_data.factors, left=0, right=0
                            )
                        ...
                        if elastic_dict:
                            ...
                            if hasattr(xs_data, "bragg_edges") and hasattr(xs_data, "factors"):
                                ...
                                print(
                                    f"[XS Plot] Loaded elastic XS using {first_key} (requested {temp_key} not found)",
                                    file=sys.stderr,
                                )
        ...
        if thermal_scattering:
            ...
            print(
                f"[XS Plot] Thermal scattering mode: material={ts_material}, temps={ts_temperatures}",
                file=sys.stderr,
            )
            ...
            try:
                ...
                if ts_data is None:
                    print(
                        json.dumps(
                            {"error": f"Could not load thermal scattering data for {ts_material}"}
                        )
                    )
                    return 1
                ...
                for temp in ts_temperatures:
                    try:
                        ...
                        energy, inelastic_xs, elastic_xs = get_thermal_scattering_xs(
                            ts_data, temp, ts_energy_min, ts_energy_max
                        )

                        if energy is None:
                            print(
                                f"[XS Plot] No thermal scattering data at {temp}K", file=sys.stderr
                            )
                            continue
                        ...
                        print(
                            f"[XS Plot] Error processing thermal scattering at {temp}K: {e}",
                            file=sys.stderr,
                        )
                        continue
        ...
        # Handle library comparison mode
            ...
            print(
                f"[XS Plot] Library comparison mode: {len(libraries)} libraries, nuclide={lib_nuclide}, reaction={lib_reaction}",
                file=sys.stderr,
            )
            ...
            for lib in libraries:
                ...
                print(
                    f"[XS Plot] Processing library '{lib_name}' from: {lib_path}", file=sys.stderr
                )
                ...
                try:
                    ...
                    if lib_path:
                        try:
                            ...
                            print(
                                f"[XS Plot] Successfully loaded library '{lib_name}' from: {lib_path}",
                                file=sys.stderr,
                            )
                        except Exception as e:
                            print(
                                f"[XS Plot] ERROR: Could not load library '{lib_name}' from {lib_path}: {e}",
                                file=sys.stderr,
                            )
                            import traceback
                    ...
                    print(
                        f"[XS Plot] Looking for nuclide {lib_nuclide} in library '{lib_name}'...",
                        file=sys.stderr,
                    )
                    nuc_data = load_nuclide_data(lib_nuclide, lib_data_library, lib_xs_dir)
                    if nuc_data is None:
                        print(
                            f"[XS Plot] ERROR: Could not load nuclide {lib_nuclide} from library '{lib_name}'",
                            file=sys.stderr,
                        )
                        continue
                    print(
                        f"[XS Plot] Found nuclide {lib_nuclide} in library '{lib_name}'",
                        file=sys.stderr,
                    )

                    # Get XS data
                    energy, xs_values = get_xs_data_for_temp(
                        nuc_data, lib_reaction, lib_temperature
                    )
                    if energy is None:
                    ...
                    if hasattr(nuc_data, "resonances") and nuc_data.resonances:
                        ...
                        if hasattr(res, "resolved") and res.resolved:
                            resolved_list = (
                                res.resolved if isinstance(res.resolved, list) else [res.resolved]
                            )
                        ...
                        if hasattr(res, "unresolved") and res.unresolved:
                            unresolved_list = (
                                res.unresolved
                                if isinstance(res.unresolved, list)
                                else [res.unresolved]
                            )
                    ...
                    curves.append(curve)
                    print(
                        f"[XS Plot] Added curve for '{lib_name}': {len(energy_filtered)} points",
                        file=sys.stderr,
                    )

                except Exception as e:
                    print(
                        f"Warning: Failed to get data from library '{lib_name}': {e}",
                        file=sys.stderr,
                    )
                    import traceback
        ...
        # Handle temperature comparison mode
            ...
            if nuc_data is None:
                print(
                    json.dumps({"error": f"Could not load data for {temp_comparison['nuclide']}"})
                )
                return 1
            ...
            for material in materials:
                ...
                for comp in components:
                    ...
                    component_data.append(
                        {"nuclide": nuc_name, "fraction": fraction, "data": nuc_data}
                    )
                ...
                for reaction_mt in parsed_reactions:
                    ...
                    first_energy, _ = get_xs_data_for_temp(
                        component_data[0]["data"], reaction_mt, temperature
                    )
                    ...
                    for comp in component_data:
                        ...
                        atomic_weight = (
                            nuc_data.atomic_weight if hasattr(nuc_data, "atomic_weight") else 1.0
                        )
                        avogadro = 6.02214076e23
        ...
        # Standard mode: individual nuclides
            ...
            for nuclide_name in nuclides:
                try:
                    ...
                    if nuc_data is None:
                        print(
                            f"[XS Plot] ERROR: Could not find cross-section data for {nuclide_name}",
                            file=sys.stderr,
                        )
                        print(
                            f"[XS Plot] Searched in: {xs_dir if xs_dir else 'cross_sections.xml library'}",
                            file=sys.stderr,
                        )
                        continue
                    ...
                    if hasattr(nuc_data, "resonances") and nuc_data.resonances:
                        res = nuc_data.resonances
                        print(
                            f"[XS Plot] Found resonance object for {nuclide_name}: {type(res)}",
                            file=sys.stderr,
                        )
                        ...
                        if hasattr(res, "resolved") and res.resolved:
                            ...
                            print(
                                f"[XS Plot] Found {len(resolved_list)} resolved region(s)",
                                file=sys.stderr,
                            )
                            for r in resolved_list:
                                print(
                                    f"[XS Plot] Resolved region: {r.energy_min} to {r.energy_max} eV",
                                    file=sys.stderr,
                                )
                                resonance_regions.append(
                                    {
                                ...
                                try:
                                    if hasattr(r, "parameters") and r.parameters is not None:
                                        df = r.parameters
                                        print(
                                            f"[XS Plot] Resonance parameters found. Type: {type(df)}",
                                            file=sys.stderr,
                                        )
                                        ...
                                        energies = (
                                            df["energy"].values if is_pandas else df["energy"]
                                        )
                                        ...
                                        print(
                                            f"[XS Plot] {len(in_range)} resonances in plot range",
                                            file=sys.stderr,
                                        )
                                        ...
                                        if len(in_range) > 100:
                                            print(
                                                "[XS Plot] Limiting 100+ resonances to top 100",
                                                file=sys.stderr,
                                            )
                                            if is_pandas:
                                                sort_col = next(
                                        ...
                                        if is_pandas:
                                            for _, row in in_range.iterrows():
                                                ...
                                                width_map = {
                                                    ...
                                                    "captureWidth": "gammaWidth",
                                                    "gammaWidth": "gammaWidth",
                                    ...
                                    print(
                                        f"[XS Plot] Error extracting parameters: {e}",
                                        file=sys.stderr,
                                    )
                                    import traceback
                        ...
                        if hasattr(res, "unresolved") and res.unresolved:
                            ...
                            print(
                                f"[XS Plot] Found {len(unresolved_list)} unresolved region(s)",
                                file=sys.stderr,
                            )
                            for ur in unresolved_list:
                                resonance_regions.append(
                        ...
                        print(
                            f"[XS Plot] No 'resonances' attribute found in nuc_data for {nuclide_name}",
                            file=sys.stderr,
                        )
                        ...
                        if hasattr(nuc_data, "urr") and nuc_data.urr:
                            ...
                            for _temp, urr_table in nuc_data.urr.items():
                                if hasattr(urr_table, "energy") and urr_table.energy is not None:
                                    ...
                                    if len(energies) > 0:
                                        ...
                                        print(
                                            f"[XS Plot] URR energy range: {energy_min_urr} to {energy_max_urr} eV",
                                            file=sys.stderr,
                                        )
                                        resonance_regions.append(
                                            {
                    ...
                    for reaction_mt in parsed_reactions:
                        try:
                            energy, xs_values = get_xs_data_for_temp(
                                nuc_data, reaction_mt, temperature
                            )
                            ...
                            print(
                                f"[XS Plot] include_uncertainty={include_uncertainty}, checking for MT={reaction_mt}",
                                file=sys.stderr,
                            )
                            if include_uncertainty:
                                print(
                                    f"[XS Plot] Calling extract_uncertainty_data for MT={reaction_mt}",
                                    file=sys.stderr,
                                )
                                uncertainty = extract_uncertainty_data(
                                    nuc_data, reaction_mt, temperature, energy_filtered
                                )
                                print(
                                    f"[XS Plot] extract_uncertainty_data returned: {uncertainty is not None}",
                                    file=sys.stderr,
                                )
                                if uncertainty:
                                    curve["uncertainty"] = uncertainty
                                    print(
                                        f"[XS Plot] Added uncertainty to curve for MT={reaction_mt}",
                                        file=sys.stderr,
                                    )
                            ...
                            print(
                                f"Warning: Failed to get reaction {reaction_mt} for {nuclide_name}: {e}",
                                file=sys.stderr,
                            )
                            continue
                    ...
                    print(
                        f"Warning: Failed to process nuclide {nuclide_name}: {e}", file=sys.stderr
                    )
                    import traceback
        ...
        result = {"curves": curves, "temperature": temperature if not temp_comparison else None}
