"""
Basic OpenMC commands (info, list, check).
"""

import json

from nuke_viz.plugin import arg, command


@command("openmc.info", help="Get statepoint info")
@arg("statepoint", help="Path to statepoint file")
def cmd_info(args):
    """Get statepoint info."""
    from plugins.openmc.lib.reader import OpenMCReader

    reader = OpenMCReader()
    info = reader.load_statepoint(args.statepoint)
    print(json.dumps(info, indent=2))
    return 0


@command("openmc.list", help="List tallies in statepoint")
@arg("statepoint", help="Path to statepoint file")
def cmd_list(args):
    """List tallies in statepoint file."""
    from plugins.openmc.lib.reader import OpenMCReader

    reader = OpenMCReader()
    tallies = reader.list_tallies(args.statepoint)
    print(json.dumps(tallies, indent=2))
    return 0


@command("openmc.check", help="Check if OpenMC integration is available")
def cmd_check(args):
    """Check if OpenMC Python module is available."""
    try:
        import openmc

        print(
            json.dumps(
                {
                    "available": True,
                    "version": openmc.__version__ if hasattr(openmc, "__version__") else "unknown",
                }
            )
        )
        return 0
    except ImportError:
        print(json.dumps({"available": False, "error": "OpenMC Python module not installed"}))
        return 1
