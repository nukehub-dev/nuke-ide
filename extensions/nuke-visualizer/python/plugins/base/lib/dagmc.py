...
def convert_h5m_to_multiblock_vtk(h5m_path: str, output_dir: str = None,
                                   include_graveyard: bool = False) -> Dict[str, Any]:
    """
...
def get_dagmc_model_info(h5m_path: str) -> Dict[str, Any]:
    ...
    for mat_name, vols in model.volumes_by_material.items():
        materials[mat_name] = {
            ...
            'volumeIds': [int(v.id) for v in vols]
        }
    ...
    for group in model.groups:
        ...
        if group_volumes:
            groups[group_name] = {
                'volumeCount': len(group_volumes),
                'volumeIds': group_volumes
            }
    ...
    for surf in sorted(model.surfaces, key=lambda s: int(s.id)):
        surfaces.append({
            ...
            'forwardVolumes': [int(v.id) for v in surf.forward_volumes] if hasattr(surf, 'forward_volumes') else [],
            'reverseVolumes': [int(v.id) for v in surf.reverse_volumes] if hasattr(surf, 'reverse_volumes') else []
        })
