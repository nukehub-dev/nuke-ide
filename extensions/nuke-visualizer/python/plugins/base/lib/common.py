...
def verify_or_find_port(
    preferred_port: int, fallback_start: int = 8080, fallback_max: int = 9000
) -> int:
...
def hex_to_rgb(hex_color: str) -> list[float] | None:
...
def calculate_camera_position(
    view_type: str, bounds: list[float]
    ...
    view_configs = {
        "isometric": (
            [cx + distance * 0.7, cy + distance * 0.7, cz + distance * 0.7],
            [cx, cy, cz],
            [0, 0, 1],
        ),
...
class UIComponents:
    ...
    def background_color_picker(
        vuetify, v_model_binding: tuple = ("background_color_hex", "#1a1a26")
    ):
    ...
    @staticmethod
    def representation_selector(
        vuetify, v_model_binding: tuple = ("representation", "Surface"), **kwargs
    ):
    ...
    @staticmethod
    def color_by_selector(
        vuetify,
        items_binding: tuple = ("available_arrays",),
        v_model_binding: tuple = ("color_by", "Solid Color"),
        **kwargs,
    ):
    ...
    @staticmethod
    def color_map_selector(
        vuetify,
        color_maps: list[str] = None,
        v_model_binding: tuple = ("color_map", "Cool to Warm"),
        **kwargs,
    ):
    ...
    @staticmethod
    def color_map_selector_short(
        vuetify, v_model_binding: tuple = ("color_map", "Plasma"), **kwargs
    ):
    ...
    @staticmethod
    def create_canvas_gadget(
        vuetify, pan_callback, zoom_callback, reset_callback=None, view_callback=None
    ):
        ...
        with vuetify.VContainer(
            ...
            html.Div(
                "Camera",
                classes="text-caption text-center mb-1 nuke-camera-title",
                style="font-weight: bold; text-transform: uppercase; letter-spacing: 1.5px; font-size: 0.65rem !important;",
            )
            ...
            def nav_tooltip(icon_name, text, **kwargs):
                with vuetify.VTooltip(bottom=True, color="#283593"):
                    ...
                    with html.Div(style="padding: 4px;"):
                        html.Div(
                            text,
                            style="font-weight: bold; border-bottom: 1px solid var(--nuke-gadget-divider); margin-bottom: 4px; padding-bottom: 2px;",
                        )
                        if "extra" in kwargs:
        ...
        def handler(representation, **kwargs):
            try:
                ...
                if display:
                    ...
                    state.appearance_update = (
                        state.appearance_update + 1 if hasattr(state, "appearance_update") else 1
                    )
            except Exception as e:
        ...
        try:
            ...
            try:
                ...
                simple.SaveScreenshot(
                    filename, view, ImageResolution=view.ViewSize, TransparentBackground=transparent
                )
                ...
                return {"success": True, "data": image_data, "format": "png", "filename": filename}
...
def create_control_panel(
    vuetify,
    server,
    show_controls_binding: tuple = ("show_controls", True),
    width: int = 320,
    theme: str = "dark",
):
