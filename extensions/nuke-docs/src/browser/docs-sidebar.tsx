...
export const DocsSidebar = (props: DocsSidebarProps): React.ReactElement => {
  const {
    nav, expanded, currentPath, searchQuery, searchResults, searchOpen, searchLoading,
    onToggleExpanded, onSelectItem, onSearch, onClearSearch, onSelectSearchResult
  } = props;

  const renderSearchDropdown = (): React.ReactNode => (
    <div className="nuke-docs-search-dropdown">
      {searchLoading ? (
        <div className="nuke-docs-search-loading"><i className={codicon('loading')} /> Searching...</div>
        ...
        <div className="nuke-docs-search-results">
          {searchResults.map(r => (
            <button
              key={r.path}
              className="nuke-docs-search-result"
              onClick={() => onSelectSearchResult(r.path)}
            >
    ...
    return (
      <div key={item.path} className="nuke-docs-nav-item">
        ...
        >
          {hasChildren ? (
            <button
              ...
              onClick={e => { e.stopPropagation(); onToggleExpanded(item.path); }}
            >
        ...
        {hasChildren && isExpanded && (
          <div className="nuke-docs-nav-children">
