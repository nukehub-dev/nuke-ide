...
export class DocsWidget extends ReactWidget {
  ...
  constructor() {
    ...
    this.md.renderer.rules.heading_open = (tokens: MarkdownIt.Token[], idx: number, options: MarkdownIt.Options, env: unknown, self: MarkdownIt.Renderer) => {
    ...
    this.md.renderer.rules.image = (tokens: MarkdownIt.Token[], idx: number, _options: MarkdownIt.Options, _env: unknown, self: MarkdownIt.Renderer) => {
      const token = tokens[idx];
    ...
    this.md.renderer.rules.link_open = (tokens: MarkdownIt.Token[], idx: number, options: MarkdownIt.Options, env: unknown, self: MarkdownIt.Renderer) => {
      const token = tokens[idx];
    ...
    this.md.renderer.rules.fence = (tokens: MarkdownIt.Token[], idx: number, _options: MarkdownIt.Options, _env: unknown, _self: MarkdownIt.Renderer) => {
      const token = tokens[idx];
  ...
  render(): React.ReactNode {
    ...
    return (
      ...
      >
        ...
        <div className="nuke-docs-main">
          ...
          <div className="nuke-docs-header">
            <button
              ...
              onClick={() => { this.state.sidebarCollapsed = !this.state.sidebarCollapsed; this.update(); }}
            >
            ...
            <nav className="nuke-docs-breadcrumbs">
              {breadcrumbs.map((item, i) => (
                <React.Fragment key={item.path}>
                  {i > 0 && <span className="nuke-docs-breadcrumb-sep"><i className={codicon('chevron-right')} /></span>}
                  <button
            ...
            <button
              className="nuke-docs-header-toggle"
              onClick={() => { this.state.tocCollapsed = !this.state.tocCollapsed; this.update(); }}
            >
        ...
        {!tocCollapsed && (
          <DocsToc
            items={this.state.tocItems}
            activeId={this.state.activeTocId}
            onSelect={this.scrollToHeading}
          />
        )}
      </div>
    );
