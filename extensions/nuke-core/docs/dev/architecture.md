...
| `StaticFrontendServer`   | `BackendApplicationServer` override that serves the built frontend (`lib/frontend`). Adds `Cache-Control: no-store` to HTML responses so redeploys propagate despite Theia's un-hashed bundle filenames; JS/CSS assets keep ETag revalidation. Must stay the only `BackendApplicationServer` binding in the app. |
