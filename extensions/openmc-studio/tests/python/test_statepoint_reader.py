...
_HAS_OPENMC = (importlib.util.find_spec('openmc') is not None
...
class TestAnalyzeKeffConvergence:
    ...
    def test_fewer_than_ten_batches(self):
        ...
        sp = {'kEffectiveByBatch': [{'batch': i + 1, 'value': v, 'stdDev': 0.01}
    ...
    def test_converged_constant_history(self):
        ...
        sp = {'kEffectiveByBatch': [{'batch': i + 1, 'value': 1.0, 'stdDev': 0.01}
                                    for i in range(20)]}
        result = analyze_keff_convergence(sp)
        assert result['success'] is True
