...
- The backend interface is consumed by the frontend as a *proxy* and implemented by the backend as a *handler*.
...
bind(OpenMCStudioBackendService).toDynamicValue(ctx => {
    ...
    const client: OpenMCStudioClient = {
        ...
        onOptimizationProgress: (event) => {
            window.dispatchEvent(new CustomEvent('openmc-optimization-progress', {
                detail: event
            }));
        },
        // ... other callbacks
