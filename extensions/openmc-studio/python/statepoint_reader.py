...
def read_statepoint(filepath: str) -> dict[str, Any]:
    ...
    if not filepath.suffix == ".h5":
        return {"success": False, "error": f"Not an HDF5 file: {filepath}"}
    ...
    try:
        ...
        if sp.run_mode == "eigenvalue":
            ...
            k_eff = sp.k_combined
            result["kEff"] = {"value": float(k_eff.nominal_value), "stdDev": float(k_eff.std_dev)}
            ...
            if hasattr(sp, "k_generation") and sp.k_generation is not None:
                ...
                for i, k in enumerate(k_gen):
                    ...
                    if hasattr(k, "nominal_value"):
                        ...
                        k_eff_by_batch.append({"batch": i + 1, "value": float(k), "stdDev": 0.0})
        ...
        for tally_id in sp.tallies:
            ...
            tally_data = {
                ...
                "nuclides": [str(n) for n in tally.nuclides]
                if hasattr(tally, "nuclides")
                else ["total"],
            }
...
def compare_statepoints(filepaths: list[str]) -> dict[str, Any]:
    ...
    for filepath in filepaths:
        ...
        if result.get("success"):
            ...
            errors.append({"file": filepath, "error": result.get("error", "Unknown error")})
        ...
        for tally_id in tally_ids:
            ...
            for r in results:
                if "tallies" in r:
                    ...
                    if matching:
                        tally_data.append({"file": r["fileName"], "tally": matching[0]})
...
def read_depletion_results(filepath: str) -> dict[str, Any]:
    ...
    try:
        import h5py
        import openmc  # noqa: F401  # availability probe: import only to detect missing dependency
        from openmc.deplete import Results
    except ImportError as e:
        return {"success": False, "error": f"Missing dependency: {e}. Please install openmc."}
    ...
    try:
        ...
        try:
            ...
            keff_values = []
            for _i, k_row in enumerate(keff_data):
                ...
                if hasattr(k_row, "__len__"):
                    ...
                    keff_values.append({"value": float(k_row), "stdDev": 0.0})
            ...
            for r in results:
                try:
                    ...
                    if hasattr(k, "__len__"):
                        keff_values.append(
                            {"value": float(k[0]), "stdDev": float(k[1]) if len(k) > 1 else 0.0}
                        )
                    else:
                        keff_values.append({"value": float(k), "stdDev": 0.0})
                except Exception:
                ...
                try:
                    ...
                    mat_display_name = (
                        ...
                        if mat_name
                            ...
                            mat_first.name
                            if hasattr(mat_first, "name") and mat_first.name
                            else mat_id_str
                        )
                    )
                    ...
                    nuclide_list = (
                        mat_first.get_nuclides() if hasattr(mat_first, "get_nuclides") else []
                    )
                    ...
                    for nuc in nuclide_list:
                        try:
                            ...
                            times_nuc, concentrations = results.get_atoms(
                                mat=mat_id_str, nuc=nuc, nuc_units="atoms", time_units="s"
                            )
                    ...
                    if nuclides:
                        nuclide_data[str(mat_id)] = {"name": mat_display_name, "nuclides": nuclides}
                except Exception:
...
def perform_statistical_tests(statepoints: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    """
    stats = {"kEffective": {}, "tallies": {}}

    # Chi-square test for k-effective consistency
    keff_data = [(sp["kEff"]["value"], sp["kEff"]["stdDev"]) for sp in statepoints if "kEff" in sp]
    ...
    if len(keff_data) >= 2:
        ...
        if total_weight > 0:
            weighted_mean = sum(v * w for v, w in zip(values, weights, strict=True)) / total_weight
...
def analyze_keff_convergence(statepoint: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(running_avg) >= 10:
        ...
        return {
            ...
            "recommendation": (
                "Converged"
                if drift_percent < 1.0 and oscillation_ratio < 0.3
                else "More batches recommended"
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Read OpenMC statepoint files for comparison")
    parser.add_argument("files", nargs="*", help="Statepoint file(s) to read")
    parser.add_argument("--compare", action="store_true", help="Compare multiple statepoints")
    parser.add_argument("--json", action="store_true", help="Output as JSON")
    parser.add_argument("--tally-id", type=int, help="Extract specific tally by ID")
    parser.add_argument("--depletion", type=str, help="Read depletion results file")
    parser.add_argument(
        "--stats", action="store_true", help="Include statistical tests in comparison"
    )
    parser.add_argument(
        "--convergence", type=str, help="Analyze k-effective convergence for a statepoint file"
    )
    ...
    # Handle single or multiple files
        ...
        if len(args.files) == 1:
            ...
            result = {
                ...
                "errors": [
                    {"file": f, "error": r.get("error")}
                    for f, r in zip(args.files, results, strict=True)
