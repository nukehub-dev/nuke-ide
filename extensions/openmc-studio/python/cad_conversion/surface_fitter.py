...
from .core import (
    SurfaceFitResult, normalize_vector, is_axis_aligned,
    distance_point_to_line, vector_subtract, dot_product,
    cross_product, vector_add, vector_scale, ransac_fit
)
...
def fit_plane(points: List[List[float]]) -> Optional[SurfaceFitResult]:
...
def fit_sphere(points: List[List[float]]) -> Optional[SurfaceFitResult]:
    ...
    points_arr = np.array(points)
    A = np.column_stack([
        2*points_arr[:, 0], 2*points_arr[:, 1], 2*points_arr[:, 2],
    ...
    try:
        ...
        deviations = [
            abs(math.sqrt(sum((p[i] - center[i])**2 for i in range(3))) - r)
            for p in points
        ]
...
def fit_cylinder(points: List[List[float]]) -> Optional[SurfaceFitResult]:
    ...
    for axis_idx in [2, 0]:  # largest eigenvalue first, then smallest
        ...
        axis_vec = normalize_vector(axis_vec.tolist() if hasattr(axis_vec, 'tolist') else list(axis_vec))
        ...
        v = normalize_vector(np.cross(axis_vec, u).tolist() if hasattr(np.cross(axis_vec, u), 'tolist') else list(np.cross(axis_vec, u)))
        ...
        A = np.column_stack([projections_arr[:, 0], projections_arr[:, 1], np.ones(len(projections_arr))])
            ...
            if max_dev < best_dev:
                ...
                best_result = SurfaceFitResult(
                    ...
                    coefficients=[float(center[0]), float(center[1]), float(center[2]),
                                 float(axis_vec[0]), float(axis_vec[1]), float(axis_vec[2]), float(r)],
                    max_deviation=max_dev,
                    center=center,
...
def fit_general_cylinder(points: List[List[float]]) -> Optional[SurfaceFitResult]:
...
def fit_cone(points: List[List[float]]) -> Optional[SurfaceFitResult]:
    ...
    for axis_vec in candidates:
        ...
        u = normalize_vector(u.tolist() if hasattr(u, 'tolist') else list(u))
        v = normalize_vector((np.cross(w, u)).tolist() if hasattr(np.cross(w, u), 'tolist') else list(np.cross(w, u)))
        u = np.array(u)
        v = np.array(v)
...
def fit_torus(points: List[List[float]]) -> Optional[SurfaceFitResult]:
...
def fit_quadric(points: List[List[float]]) -> Optional[SurfaceFitResult]:
    ...
    try:
        ...
        deviations = []
        for p in points:
            val = (coeffs_list[0]*p[0]**2 + coeffs_list[1]*p[1]**2 + coeffs_list[2]*p[2]**2 +
                   coeffs_list[3]*p[0]*p[1] + coeffs_list[4]*p[0]*p[2] + coeffs_list[5]*p[1]*p[2] +
                   coeffs_list[6]*p[0] + coeffs_list[7]*p[1] + coeffs_list[8]*p[2] + coeffs_list[9])
            # Normalize by gradient magnitude for geometric distance approx
            grad = np.array([
                2*coeffs_list[0]*p[0] + coeffs_list[3]*p[1] + coeffs_list[4]*p[2] + coeffs_list[6],
                2*coeffs_list[1]*p[1] + coeffs_list[3]*p[0] + coeffs_list[5]*p[2] + coeffs_list[7],
                2*coeffs_list[2]*p[2] + coeffs_list[4]*p[0] + coeffs_list[5]*p[1] + coeffs_list[8],
            ])
...
def classify_and_fit(points: List[List[float]],
                     gmsh_surface_type: str = "Unknown",
                     tolerance: float = 0.001) -> Optional[SurfaceFitResult]:
