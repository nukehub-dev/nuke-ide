...
try:
    import numpy as np  # noqa: F401  # availability probe: sets HAS_NUMPY
        ...
        for solid_dim, solid_tag in solids:
            ...
            for surf_dim, signed_tag in boundary:
                ...
                if fit_result.warning:
                    result["warnings"].append(f"Surface {signed_tag}: {fit_result.warning}")
...
def main():
    ...
    parser.add_argument(
        "--unit-factor", type=float, default=1.0, help="Unit conversion factor (default: 1.0)"
    )
    parser.add_argument(
        "--tolerance",
        type=float,
    ...
    parser.add_argument(
        "--scale", type=float, default=1.0, help="Additional scale factor (default: 1.0)"
    )
    parser.add_argument(
        "--material-id", type=int, default=None, help="Material ID for imported cells"
    )
    parser.add_argument("--universe-id", type=int, default=0, help="Universe ID for imported cells")
    parser.add_argument("--output-json", action="store_true", help="Output result as JSON")
    parser.add_argument("--force-dagmc", action="store_true", help="Force DAGMC conversion")
    parser.add_argument(
        "--force-csg", action="store_true", help="Force CSG conversion even with NURBS"
    )
    parser.add_argument(
        "--dagmc-output", type=str, default=None, help="Output path for DAGMC .h5m file"
    )
    parser.add_argument(
        "--faceting-tol",
        type=float,
