    ...
    try:
        ...
        for vol in model.volumes:
            ...
            try:
                ...
                bbox = {"min": [float(x) for x in bounds[0]], "max": [float(x) for x in bounds[1]]}
            except Exception:
                bbox = None
        ...
        if model.volumes:
            ...
            if all_bounds:
                ...
                overall_bbox = {"min": mins.min(axis=0).tolist(), "max": maxs.max(axis=0).tolist()}
            else:
    ...
    if args.output_json:
        ...
        if result["success"]:
            ...
            for mat, info in result["materials"].items():
                print(
                    f"    {mat}: {info['volumeCount']} volumes, {info['totalTriangles']:,} triangles"
                )
            print("\n  Bounding Box:")
