...
- `nukelab-integration` owns the idle-activity contract with the NukeLab hub: `NukeLabActivityContribution` POSTs `/api/nukelab/activity` (via `Endpoint`) every 60s only after recent deliberate input, so the container auth sidecar sees real IDE usage; an untouched IDE tab must not keep the server alive.
