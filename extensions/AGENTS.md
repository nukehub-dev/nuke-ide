...
- `nuke-core` owns the `BackendApplicationServer` override (`src/node/static-frontend-server.ts`) that serves the built frontend with `Cache-Control: no-store` on HTML so redeploys propagate despite Theia's un-hashed bundle filenames; keep it the only such binding in the app.
